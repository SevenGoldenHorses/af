...
var fs = require('fs')
var logger = fs.createWriteStream('report-7GoldenHorses.txt', {
...
function BOMBA(tweet) {
	...
	if ( tweet.user.screen_name !== '7GoldenHorses' ) {
...
//funzione per convertire screen_name in ID e poi fare l'auto-follow


T.get('users/lookup',{
  //inserischi screen_name presi da internet
  screen_name: 'medichainonline,vestarin'
}, function autoFolllow (err, data, response) {
    for (var i = 0; i < data.length; i++) {
      // fa il follow dell-id dell'oggettp di ritorno da Twitter
      T.post('friendships/create',{id: data[i].id_str } ); // i valori provenienti da oggetto si ma non le var con stringhe dentro
    }
  } //fine funzione
); //fine programma
